...
if (isset($_GET['id'])) {
    ...
    foreach ($x as $key => $value) {
        $query = "SELECT p_name,importPrice,unity,supllier_id FROM product WHERE _id = $value[product_id]";
        $result = $connect->query($query);
        $product_recipt[$key] = mysqli_fetch_assoc($result);
        $product_recipt[$key]["quantity"] = $value['quantity'];
        $product_recipt[$key]["total_price"] = $value['quantity'] * $product_recipt[$key]["importPrice"];
    }
